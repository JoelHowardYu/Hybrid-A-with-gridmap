...
namespace planning
{
// 标准化角度函数，将角度转换到 [-π, π) 范围内
double NormalizeAngle(const double angle) {
  // 使用 std::fmod 函数计算 (angle + π) 对 2π 取模的结果
  double a = std::fmod(angle + M_PI, 2.0 * M_PI);
  // 如果结果小于 0，则加上 2π 使其落在 [0, 2π) 范围内
  if (a < 0.0) {
    a += (2.0 * M_PI);
  }
  // 最后减去 π，将角度转换到 [-π, π) 范围内
  return a - M_PI;
}
...
std::pair<double, double> Cartesian2Polar(double x, double y) {
  ...
  // 计算极角 theta，使用 std::atan2 函数计算 y/x 的反正切值
  double theta = std::atan2(y, x);
  // 返回极坐标 (r, theta)
  return std::make_pair(r, theta);
}

// 计算 Reed-Shepp 路径中的 tau 和 omega 参数
std::pair<double, double> ReedShepp::calc_tau_omega(const double u,
                                                    const double v,
                                                    const double xi,
                                                    const double eta,
                                                    const double phi) {
  // 计算 delta，即 u 和 v 的差值，并标准化到 [-π, π) 范围内
...
bool ReedShepp::SCS(const double x, const double y, const double phi,
  ...
  // 定义 SLS 路径的类型
  char SLS_types[] = "SLS";
  // 如果 SLS 参数有效且设置路径失败，输出错误信息并返回 false
  if (SLS_param.flag &&
  ...
  // 定义 SRS 路径的类型
  char SRS_types[] = "SRS";
  // 如果 SRS 参数有效且设置路径失败，输出错误信息并返回 false
  if (SRS_param.flag &&
